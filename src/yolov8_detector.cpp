...
std::vector<Detection> Yolov8Detector::infer(const float* data, std::size_t count,
    ...
    // Native Ultralytics exports use [1,C,N]. If both interpretations are
    // mathematically possible, choose the smaller dimension as channels;
    // this avoids silently treating a small-prediction model as transposed.
    if (channels_first && rows_first) {
